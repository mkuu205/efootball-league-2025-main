// Advanced Fixture Management System - SUPABASE ONLY VERSION
import { getData, saveData, DB_KEYS, getSupabase, showNotification, updateFixture, deleteFixture, addFixture } from './database.js';
...
class FixtureManager {
    ...
    init() {
        console.log('Fixture Manager initialized - Using Supabase');
    ...
    // Generate optimized fixtures - SUPABASE ONLY
    async generateOptimizedFixtures() {
        try {
            ...
            if (!Array.isArray(players)) {
                ...
                showNotification('Error: Invalid players data format', 'error');
                return;
            }
            ...
            if (confirm('This will replace all existing fixtures with an optimized schedule. Continue?')) {
                ...
                setTimeout(async () => {
                    try {
                        ...
                        showNotification('✅ Optimized fixtures generated successfully in Supabase!', 'success');
            ...
            showNotification('Error loading players data', 'error');
        }
    }
    ...
    createOptimizedFixtures(players) {
        ...
        if (!Array.isArray(players) || players.length === 0) {
            throw new Error('Invalid players data: players must be a non-empty array');
        }
        ...
        for (let i = 0; i < players.length; i++) {
            for (let j = i + 1; j < players.length; j++) {
                ...
                if (!players[i] || !players[j] || !players[i].id || !players[j].id) {
                    console.warn('Skipping invalid player pair:', players[i], players[j]);
                    continue;
                }
                ...
                matchPairs.push({
                    ...
                    strengthDiff: Math.abs((players[i].strength || 50) - (players[j].strength || 50))
                });
            }
        }

        if (matchPairs.length === 0) {
            throw new Error('No valid player pairs found to generate fixtures');
        }
        ...
        schedule.forEach((daySchedule, dayIndex) => {
            ...
            daySchedule.forEach((match, matchIndex) => {
                const timeString = this.timeSlots[matchIndex % this.timeSlots.length];
                ...
                fixtures.push({
                    ...
                    date: dateString,
                    time: timeString, // Keep as string - database should handle this as text
                    venue: venue,
                    played: false,
                    is_home_leg: true,
                    status: 'scheduled',
        ...
        console.log(`Generated ${fixtures.length} fixtures for ${players.length} players`);
        return fixtures;
    }
    ...
    createBalancedSchedule(matchPairs, players) {
        ...
        if (!Array.isArray(matchPairs)) {
            throw new Error('matchPairs must be an array');
        }
        if (!Array.isArray(players)) {
        ...
        let dayIndex = 0;
        let iterations = 0;
        const maxIterations = matchPairs.length * 10; // Prevent infinite loops
        
        while (matchPairs.length > 0 && iterations < maxIterations) {
            iterations++;
        ...
        if (iterations >= maxIterations) {
            console.warn('Fixture generation reached max iterations, some matches may not be scheduled');
        }
    ...
    canScheduleMatch(dayMatches, match, playerMatchCount, dayIndex) {
        // Validate inputs
        if (!match || !match.homePlayer || !match.awayPlayer) {
            return false;
        }
    ...
    async saveOptimizedFixtures(fixtures) {
        try {
            if (!Array.isArray(fixtures) || fixtures.length === 0) {
                throw new Error('No fixtures to save');
            }

            // Clear existing fixtures first
            const existingFixtures = await getData(DB_KEYS.FIXTURES);
            console.log(`Clearing ${existingFixtures.length} existing fixtures`);
            
            for (const fixture of existingFixtures) {
                await deleteFixture(fixture.id);
            }
            
            // Save new fixtures to Supabase
            console.log(`Saving ${fixtures.length} new fixtures to Supabase`);
            for (const fixture of fixtures) {
                await addFixture(fixture);
            }
            
            console.log('✅ Optimized fixtures saved to Supabase');
        } catch (error) {
            console.error('Error saving optimized fixtures:', error);
            throw error;
        }
    }
    ...
    detectDateConflicts(fixtures = null) {
        ...
        dateMap.forEach((dayFixtures, date) => {
            ...
            dayFixtures.forEach(fixture => {
                if (!fixture.home_player_id || !fixture.away_player_id) return;
    ...
    getNextFixtureId() {
        ...
        const maxId = Math.max(...fixtures.map(f => f.id || 0));
        return maxId + 1;
    }
    ...
    generateFixtureReport() {
        ...
        const report = {
            ...
            summary: {
                ...
                playedFixtures: validFixtures.filter(f => f.played).length,
                upcomingFixtures: validFixtures.filter(f => !f.played).length,
                totalPlayers: validPlayers.length,
                completionPercentage: validFixtures.length > 0 ? 
                    (validFixtures.filter(f => f.played).length / validFixtures.length) * 100 : 0
            },
            venueUsage: this.getVenueUsage(validFixtures),
            playerSchedules: this.getPlayerSchedules(validPlayers, validFixtures),
            conflicts: this.detectDateConflicts(validFixtures),
            recommendations: this.getSchedulingRecommendations(validPlayers, validFixtures),
            upcomingSchedule: this.getUpcomingSchedule(validFixtures, 7) // Next 7 days
        };
    ...
    getSchedulingRecommendations(players, fixtures) {
        ...
        if (!Array.isArray(players) || !Array.isArray(fixtures)) {
        ...
        const conflicts = this.detectDateConflicts(fixtures);
    ...
    getUpcomingSchedule(fixtures, days = 7) {
        ...
        const upcoming = fixtures
            .filter(f => !f.played && new Date(f.date || 0) <= endDate)
            .sort((a, b) => new Date(a.date || 0) - new Date(b.date || 0));
...
function showRescheduleTool() {
    ...
    if (!Array.isArray(fixtures) || !Array.isArray(players)) {
        showNotification('Error loading fixtures or players data', 'error');
        return;
    }
    ...
    let fixtureOptions = upcomingFixtures.map(f => {
        ...
        const homeName = homePlayer ? homePlayer.name : 'Unknown';
        const awayName = awayPlayer ? awayPlayer.name : 'Unknown';
        return `<option value="${f.id}">${homeName} vs ${awayName} (${f.date} ${f.time})</option>`;
    }).join('');
...
async function performReschedule() {
    ...
    const fixtures = getData(DB_KEYS.FIXTURES);
    if (!Array.isArray(fixtures)) {
        showNotification('Error loading fixtures data', 'error');
        return;
    }
    ...
    try {
        ...
        showNotification('Fixture rescheduled successfully in Supabase!', 'success');
