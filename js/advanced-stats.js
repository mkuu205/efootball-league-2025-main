...
class AdvancedStatistics {
    ...
    updateComparison() {
        ...
        const comparison = this.generatePlayerComparison(player1Id, player2Id);
        ...
        return `
            <div class="player-comparison">
                ...
                <div class="mt-4">
                    ...
                    ${this.generateHeadToHeadRecord(player1Id, player2Id)}
                </div>
            </div>
        `;
    }
    ...
    generateHeadToHeadRecord(player1Id, player2Id) {
        ...
        headToHead.forEach(result => {
            const isPlayer1Home = result.homePlayerId === player1Id;
            const player1Score = isPlayer1Home ? result.homeScore : result.awayScore;
            const player2Score = isPlayer1Home ? result.awayScore : result.homeScore;
    ...
    loadTeamStats() {
        ...
        let html = '';
        teams.forEach(team => {
            const stats = this.analyzeTeamPerformance(team);
    ...
    getTeamOverview(results, teamName) {
        ...
        results.forEach(result => {
            ...
            overview.goalsFor += teamScore;
            overview.goalsAgainst += opponentScore;
    ...
    loadTournamentLeaders() {
        ...
        const bestDefense = this.getBestDefense(players, results);
        // Calculate best form
        const bestForm = this.getBestForm(players, results);
        ...
        container.innerHTML = `
            <div class="list-group list-group-flush bg-transparent">
                ...
                <div class="list-group-item bg-transparent text-light d-flex justify-content-between align-items-center">
                    <span>⚡ Most Wins</span>
                    <span class="text-primary">${this.getMostWins(players, results)[0]?.name || 'N/A'} (${this.getMostWins(players, results)[0]?.wins || 0} wins)</span>
                </div>
            </div>
        `;
    }

    getTopScorers(players, results) {
    ...
    getBestDefense(players, results) {
        const playerDefense = players.map(player => {
            let goalsConceded = 0;
            results.forEach(result => {
                if (result.homePlayerId === player.id) goalsConceded += result.awayScore;
                if (result.awayPlayerId === player.id) goalsConceded += result.homeScore;
            });
    ...
    getBestForm(players, results) {
        const playerForms = players.map(player => {
            const stats = this.analyzePlayerPerformance(player.id);
    ...
    loadAdvancedMetrics() {
        ...
        const drawPercentage = results.length > 0 ? 
            ((results.filter(r => r.homeScore === r.awayScore).length / results.length) * 100).toFixed(1) : 0;
